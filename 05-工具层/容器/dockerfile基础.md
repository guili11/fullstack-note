...
**Q:多阶段构建镜像为什么可以减小镜像体积呢？使用多阶段构造后需要注意的问题有哪些呢？**

多阶段构建通过将编译阶段和运行阶段分离来减小镜像体积。编译阶段使用完整环境生成可执行文件，运行阶段只引入轻量基础镜像并复制最终产物，从而省掉编译工具和中间文件。

注意事项：

阶段隔离，需要显式将编译生成的可执行文件复制到运行阶段。

**复制文件到新阶段：**除了可执行文件，如果程序依赖配置文件、证书、静态资源等，也必须复制到最终镜像。

编译阶段只能生成代码（go）相关产物，外部依赖（.env .db）需单独处理。
